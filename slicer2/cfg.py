...
def extract_cfgs(file_path: str) -> List[Dict[str, List[List[int]]]]:
    ...
    (
        ...
        node_id_to_ln,
        functions,
    ) = extract_nodes_with_location_info(nodes)
    ICFG_edges = set()
    for edge in edges:
        ...
        if True:  # edge_type in ['IS_AST_PARENT', 'FLOWS_TO']:
            ...
            if (
                start_node_id not in node_id_to_ln.keys()
    ...
    for idx, entry_ln in enumerate(functions):
        ...
        while entry_ln not in ICFG._node:
            if (
                idx + 1 < len(functions) and entry_ln >= functions[idx + 1]
            ) or entry_ln > max_ln:
                is_empty = True
                break
            entry_ln += 1
...
def d2a_cfg_label(dataset, method, gen_csv: bool = False):
    source_code_dir = (
        "/home/chengxiao/dataset/vul/d2a/extracted_code_and_slice/code/d2a_resources"
    )
...
def generate_cfg_d2a():
    ...
    raw_data_path = os.path.join(
        config.data_folder, config.name, config.dataset.name, "raw.json"
    )
    cfg_out_path = os.path.join(
        config.data_folder, config.name, config.dataset.name, "all.json"
    )
    ...
    for bug_type in os.listdir(cfg_path):
        if os.path.isdir(os.path.join(cfg_path, bug_type)):
            ...
            if os.path.exists(jsonl):
                ...
                with open(
                    cfg_out_path + "/all.json", "w", encoding="utf-8", errors="ignore"
                ) as f:
