...
def get_data(root):
    ...
    for cpg in cpg_list:
        ...
        for slice_ln in call_lines:
            pdg_slice_lines = create_forward_slice(combined_graph, slice_ln) + create_backward_slice(combined_graph, slice_ln)
            pdg_slice_lines_uq = sorted(list(set(pdg_slice_lines)))
            call_slices_pdg.append(pdg_slice_lines_uq)

        array_slices_pdg = []
        for slice_ln in array_lines:
            pdg_slice_lines = create_forward_slice(combined_graph, slice_ln) + create_backward_slice(combined_graph, slice_ln)
            pdg_slice_lines_uq = sorted(list(set(pdg_slice_lines)))
            array_slices_pdg.append(pdg_slice_lines_uq)

        arith_slices_pdg = []
        for slice_ln in arithmatic_lines:
            pdg_slice_lines = create_forward_slice(combined_graph, slice_ln) + create_backward_slice(combined_graph, slice_ln)
            pdg_slice_lines_uq = sorted(list(set(pdg_slice_lines)))
            arith_slices_pdg.append(pdg_slice_lines_uq)

        ptr_slices_pdg = []
        for slice_ln in ptr_lines:
            pdg_slice_lines = create_forward_slice(combined_graph, slice_ln) + create_backward_slice(combined_graph, slice_ln)
            pdg_slice_lines_uq = sorted(list(set(pdg_slice_lines)))
            ptr_slices_pdg.append(pdg_slice_lines_uq)

        data_instance = {
            'file_path': cpg,
            'call_slices': call_slices_pdg,
            'array_slices': array_slices_pdg,
            'arith_slices': arith_slices_pdg,
            'ptr_slices': ptr_slices_pdg,
        }
...
def get_data_d2a(root):
    ...
    for cpg in cpg_list:
        ...
        for slice_ln in call_lines:
            ...
            pdg_slice_lines = forward_sliced_lines
            pdg_slice_lines.extend(backward_sliced_lines)
            pdg_slice_lines = sorted(list(set(pdg_slice_lines)))

            ddg_slice_lines = create_backward_slice(data_graph, slice_ln)
            vdp_for_lines = create_forward_slice(data_graph, slice_ln)
            ddg_slice_lines.extend(vdp_for_lines)
            ddg_slice_lines_uq = sorted(list(set(ddg_slice_lines)))

            call_slices.append(ddg_slice_lines_uq)
            call_slices_pdg.append(pdg_slice_lines)

            all_slices.append(pdg_slice_lines)
            all_slices_vd.append(ddg_slice_lines_uq)
        ...
        for slice_ln in array_lines:
            ...
            array_slices.append(ddg_slice_lines_uq)
            array_slices_pdg.append(pdg_slice_lines)
            all_slices.append(pdg_slice_lines)
            all_slices_vd.append(ddg_slice_lines_uq)
        ...
        for slice_ln in arithmatic_lines:
            ...
            ddg_slice_lines.extend(vdp_for_lines)
            ddg_slice_lines_uq = sorted(list(set(ddg_slice_lines)))
            arith_slices.append(ddg_slice_lines_uq)
            arith_slices_pdg.append(pdg_slice_lines)
            all_slices.append(pdg_slice_lines)
            all_slices_vd.append(ddg_slice_lines_uq)
        ...
        for slice_ln in ptr_lines:
            ...
            ddg_slice_lines.extend(vdp_for_lines)
            ddg_slice_lines_uq = sorted(list(set(ddg_slice_lines)))
            ptr_slices.append(ddg_slice_lines_uq)
            ptr_slices_pdg.append(pdg_slice_lines)
            all_slices.append(pdg_slice_lines)
            all_slices_vd.append(ddg_slice_lines_uq)
        ...
        data_instance = {
            ...
            'call_slices_sy': call_slices_pdg,
            'array_slices_vd': array_slices,
            'array_slices_sy': array_slices_pdg,
            'arith_slices_vd': arith_slices,
            'arith_slices_sy': arith_slices_pdg,
            'ptr_slices_vd': ptr_slices,
            'ptr_slices_sy': ptr_slices_pdg,
