...
def removeComment(inputstr):
    ...
    patternExp = (
        literalStringExp + "|" + singleLineCommentExp + "|" + multiLinecommentExp
    )
    codeString = re.sub(
        patternExp, replaceComment, inputstr, 0, re.MULTILINE | re.DOTALL
    )
    return codeString
            ...
            for bc in bcs:
                ...
                with open(jsonPath, "r", encoding="utf-8", errors="ignore") as f:
                    ...
                    for idx in range(len(cfgs)):
                        cfg = cfgs[idx]
                        cfg["nodes-line-sym"] = tokenize_gadget_tolist(
                            clean_gadget(cfg["nodes-line"])
                        )
